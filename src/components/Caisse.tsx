      ...
      <div className="card space-y-3 p-4">
        ...
        {lines.length === 0 ? (
          <p className="py-6 text-center text-sm text-[var(--foreground)]/50">
            Touchez un produit ci-dessous pour l&apos;ajouter au ticket.
          </p>
        ) : (
          <div className="max-h-72 space-y-2 overflow-y-auto">
            {lines.map((line) => (
              <div key={line.id} className="space-y-2 rounded-xl bg-[var(--color-sand-50)] p-2.5">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium leading-tight">{line.name}</p>
                    <p className="text-xs text-[var(--foreground)]/50">{formatPrice(line.price)} / unité</p>
                  </div>
                  <p className="shrink-0 text-sm font-bold">{formatPrice(line.price * line.qty)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => changeQty(line.id, -1)} className="btn btn-secondary btn-icon tap-target !p-1.5">
                    <Minus size={14} />
                  </button>
                  <span className="w-6 text-center text-sm font-bold">{line.qty}</span>
                  <button onClick={() => changeQty(line.id, 1)} className="btn btn-primary btn-icon tap-target !p-1.5">
                    <Plus size={14} />
                  </button>
                  <button
                    onClick={() => removeLine(line.id)}
                    className="btn btn-danger btn-icon tap-target ml-auto !p-1.5"
        ...
        <div className="space-y-1 border-t border-[var(--border)] pt-3">
          ...
          <div className="flex items-center justify-between text-lg font-extrabold text-[var(--color-palm-900)]">
